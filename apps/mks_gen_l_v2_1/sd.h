...
#define SPI0_MOSI_PIN							PB2   // SDA pin
#define SPI0_SCK_PIN							PB1   // SCK pin
...
static uint8_t sd_command_ACMD41(void);
...
//	CARD_DETECT_DDR &= ~(1 << CARD_DETECT_PIN);
	...
	SPCR &= ~(1 << SPR01); // set clock rate fosc/2
...
static void SPI_Init(void){
	...
	SPSR = (1 << 0);
	SPCR |= (1 << SPR01); // set clock rate fosc/32
}
