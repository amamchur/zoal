...
TEST(hd44780, uno_lcd_sheald) {
    ...
    EXPECT_EQ(c.values[0], cas_value(0x00, 0x00, 0x00)); // Dummy for GPIO port clock on
    EXPECT_EQ(c.values[1], cas_value(0x24, 0x00, 0x03)); // DDRB |= 0x03
    EXPECT_EQ(c.values[2], cas_value(0x2A, 0x00, 0xF0)); // DDRD |= 0xF0
    EXPECT_EQ(c.values[3], cas_value(0x25, 0x03, 0x00)); // PORTB ~= 0x03
    EXPECT_EQ(c.values[4], cas_value(0x2B, 0xC0, 0x10)); // PORTD = (PORTD & ~0xC0) | 0x10;
}
