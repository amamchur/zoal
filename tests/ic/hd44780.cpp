...
TEST(hd44780, uno_lcd_sheald) {
    ...
    EXPECT_EQ(c.values[0], cas_value(0x24, 0x00, 0x03, sizeof(uint8_t))); // DDRB |= 0x03
    EXPECT_EQ(c.values[1], cas_value(0x2A, 0x00, 0xF0, sizeof(uint8_t))); // DDRD |= 0xF0
    EXPECT_EQ(c.values[2], cas_value(0x25, 0x03, 0x00, sizeof(uint8_t))); // PORTB ~= 0x03
    EXPECT_EQ(c.values[3], cas_value(0x2B, 0xC0, 0x10, sizeof(uint8_t))); // PORTD = (PORTD & ~0xC0) | 0x10;
}
