...
namespace zoal { namespace arch { namespace avr {
    ...
    public:
        ...
        using clear_counter_flag = callable_cas_list<type_list<typename TIFRx<Timer>::template cas<1 << TOVx, 0>>>;

        template<uint8_t Timer, uint8_t Channel>
        using clear_channel_flag = callable_cas_list<type_list<typename type_at_index<
            //
            Channel,
